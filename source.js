...
//#step - 3 - Setting tracks for when user joins
let localTracks = {
    audioTrack:null,
    videoTrack:null
}

//#step-4 - Want to hold state for users audio and video so user can mute and hide
let localTrackState = {
    audioTrackMuted:false,
    videoTrackMuted:false
}

//#step-5 - Set remote tracks to store other users
let remoteTracks = {}


document.getElementById('joining-btn').addEventListener('click', async () => {
    config.uid = document.getElementById('user').value
    await joinStreams()
    document.getElementById('joining-wrapper').style.display = 'none'
...
document.getElementById('leave-btn').addEventListener('click', async () => {
    ...
    document.getElementById('joining-wrapper').style.display = 'block'
...
let joinStreams = async () => {
    ...
    client.on("volume-indicator", function(evt){
        for (let i = 0; evt.length > i; i++){
            ...
            if(volume > 0){
                document.getElementById(`volume-${speaker}`).src = './bharath intern/volume-on.svg'
            }else{
                document.getElementById(`volume-${speaker}`).src = './bharath intern/volume-off.svg'
            }
    ...
    let player = `<div class="video-containers" id="video-wrapper-${config.uid}">
                        <p class="user-uid"><img class="volume-icon" id="volume-${config.uid}" src="./bharath intern/volume-on.svg" /> ${config.uid}</p>
...
let handleUserJoined = async (user, mediaType) => {
    ...
    if (mediaType === 'video'){
        ...
        player = `<div class="video-containers" id="video-wrapper-${user.uid}">
                        <p class="user-uid"><img class="volume-icon" id="volume-${user.uid}" src="./bharath intern/volume-on.svg" /> ${user.uid}</p>
